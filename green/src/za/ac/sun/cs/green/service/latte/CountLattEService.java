...
public class CountLattEService extends CountService {
	...
	private class HMatrix {
		...
		private Apint processInput(String input) {
			if (store == null) {
				return new Apint(invokeLattE(input)).multiply(correction);
			} else {
				String count = store.getString(input);
				if (count == null) {
					count = invokeLattE(input);
		...
		private String invokeLattE(String input) {
			// System.out.println(">>> INVOKING LATTE:");
			...
			if (i >= 0) {
				int j = i + ANSWER_PATTERN.length();
				// System.out.println("<<< " + result.substring(j,
				// result.indexOf(')', j)));
				String tmp = result.substring(j, result.indexOf(')', j));
				assert tmp.equals(resultFromFile) : "File and Terminal Output mismatch";
				return tmp;
			} else if (resultFromFile != null) {
